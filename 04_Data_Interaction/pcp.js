...
function brush() {
  ...
  // the overall idea is to create a list of all brushed axes and record their brushed ranges respectively
  // Then you go through the "foreground" selection created previously in line 44 and change the appearance for
  // those polylines gone through the ranges of all brushed axes. Now let's start:

  var actives = [];

  // You can identify those axes that have been brushed by:

  svg.selectAll(".brush").filter(function (d) {
    return d3.brushSelection(this);
  })
    .each(function (d) { // followed by recording down the ranges that have been brushed with:
      // -- push the dimensions and brushed extend of d into a list
      actives.push({
        dimension: d,
  ...
  // Now that you have collect the dimensions and extend of all brushed dimensions
  // you can use the appeearance of those polylines that fall into the extents by:
  foreground.style("opacity",

    function (d) {
      
      
      // -- check if the polyline's coordinates in those dimensions
      // are within the brushed erxtend. Here 'd' is each of the polylines
      // -- the function should return an opacity value, meaning to set the opacity high if brushed, and low if now.

      if (actives.every(function (active) {
        var dim = active.dimension;
        var pos = y[dim](d[dim]);
        // test if point is within extents for each active brush
        rs = active.extent[0] <= pos && pos <= active.extent[1];
